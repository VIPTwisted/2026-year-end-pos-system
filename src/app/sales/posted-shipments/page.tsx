...
export default function PostedSalesShipmentsPage() {
  const [shipments, setShipments] = useState<PostedShipment[]>([])
  ...
  const filtered = shipments.filter(s => {
    ...
    return (
      ...
      (s.sellToCustomerName ?? '').toLowerCase().includes(q) ||
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800 bg-[#0f1829]">
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-zinc-400 uppercase tracking-wide">Ship-to Address</th>
              ...
              <tbody className="divide-y divide-zinc-800/60">
                ...
                {!loading && filtered.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-10 text-center text-zinc-600">No posted shipments found</td></tr>
                )}
                {filtered.map(s => (
                  <tr key={s.id} className="hover:bg-zinc-800/30 transition-colors">
                    <td className="px-4 py-3">
                      <span className="text-blue-400 font-mono text-xs font-medium">
                        {s.shipmentNo.length > 14 ? s.shipmentNo.slice(-14) : s.shipmentNo}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-zinc-300 text-xs">{s.shipToName ?? '—'}</td>
                    <td className="px-4 py-3 text-zinc-400 text-xs">{s.shipToAddress ?? '—'}</td>
