...
import { Plus, ChevronDown, Search, Filter, FileText } from 'lucide-react'
...
const STATUSES = ['All', 'Open', 'Released', 'Posted', 'Cancelled']
    ...
    <div className="min-h-[100dvh] bg-[#0f0f1a] text-zinc-100">
      ...
      <div className="border-b border-zinc-800 bg-[#0f0f1a] px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-zinc-400">
          ...
          <span>/</span>
          <span className="text-zinc-200">Sales Invoices</span>
        </div>
      ...
      <div className="border-b border-zinc-800 bg-[#16213e] px-6 py-3">
        <h1 className="text-base font-semibold text-zinc-100 mb-3">Sales Invoices</h1>
        <div className="flex items-center gap-1 flex-wrap">
          ...
          <button onClick={postSelected} disabled={selected.length === 0}
          ...
          <div className="bg-[#16213e] border border-zinc-800 rounded-lg overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800 bg-[#0f1829]">
                  <th className="w-10 px-4 py-2.5">
                    <input type="checkbox" checked={allChecked} onChange={() => setSelected(allChecked ? [] : filtered.map(i => i.id))} className="accent-blue-500" />
                  </th>
              ...
              <tbody className="divide-y divide-zinc-800/60">
                {loading && <tr><td colSpan={9} className="px-4 py-10 text-center text-zinc-600">Loading...</td></tr>}
                {!loading && filtered.length === 0 && <tr><td colSpan={9} className="px-4 py-10 text-center text-zinc-600">No invoices found</td></tr>}
                {filtered.map(inv => {
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-zinc-800/30 transition-colors group">
                      <td className="px-4 py-3">
                        <input type="checkbox" checked={selected.includes(inv.id)} onChange={() => toggleSelect(inv.id)} className="accent-blue-500" />
                      </td>
                      <td className="px-4 py-3">
                        <Link href={`/sales/invoices/${inv.id}`} className="text-blue-400 hover:text-blue-300 font-mono text-xs font-medium">
                          {inv.invoiceNumber.length > 12 ? inv.invoiceNumber.slice(-12) : inv.invoiceNumber}
                        </Link>
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <span className={cn('px-2 py-0.5 rounded text-xs font-medium', STATUS_COLORS[inv.status] ?? 'bg-zinc-700/50 text-zinc-400')}>
                          {inv.status}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link href={`/sales/invoices/${inv.id}`} className="text-xs text-zinc-500 hover:text-zinc-200 opacity-0 group-hover:opacity-100 transition-opacity">Edit</Link>
                      </td>
                    </tr>
                  )
                })}
