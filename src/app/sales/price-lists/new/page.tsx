...
export default function NewSalesPriceListPage() {
  const router = useRouter()
  const [form, setForm] = useState({
    code: '',
    description: '',
    assignToType: 'All Customers',
    assignTo: '',
    currency: 'USD',
    startingDate: '',
    endingDate: '',
  ...
  const [lines, setLines] = useState<PriceLine[]>([{ ...EMPTY_LINE }])
  ...
  const updateLine = (i: number, k: keyof PriceLine, v: string | number | boolean) =>
  ...
  const save = async () => {
    ...
    setSaving(true)
    try {
      const res = await fetch('/api/sales/price-lists', {
        method: 'POST',
      ...
      setTimeout(() => router.push(`/sales/price-lists/${data.id}`), 700)
    } catch (e) {
      notify(String(e), 'err')
      setSaving(false)
    }
    ...
    <div className="flex flex-col min-h-[100dvh] bg-[#0f0f1a]">
      <TopBar
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link href="/sales/price-lists" className="px-3 py-1.5 rounded text-xs font-medium bg-zinc-700 text-zinc-300 hover:bg-zinc-600 transition-colors">Cancel</Link>
      ...
      <div className="flex-1 p-6 max-w-6xl mx-auto w-full space-y-6">
          ...
          <div className="p-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              ...
              <input type="text" value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))}
                placeholder="SPL-001" className={FIELD} />
            </div>
            <div className="md:col-span-2">
              <label className={LABEL}>Description</label>
              <input type="text" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                placeholder="e.g. Retail Price List 2026" className={FIELD} />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={form.assignToType} onChange={e => setForm(f => ({ ...f, assignToType: e.target.value }))} className={FIELD}>
                <option>All Customers</option><option>Customer</option>
                <option>Customer Price Group</option><option>Customer Disc. Group</option>
              </select>
            </div>
            <div>
              <label className={LABEL}>Assign-to</label>
              <input type="text" value={form.assignTo} onChange={e => setForm(f => ({ ...f, assignTo: e.target.value }))}
                placeholder="Customer No. or group" className={FIELD} />
            </div>
            <div>
        ...
        <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg overflow-hidden">
                  ...
                  <tr key={i} className="hover:bg-zinc-800/10 transition-colors">
                    <td className="px-3 py-2">
                      <select value={l.productType} onChange={e => updateLine(i, 'productType', e.target.value)}
                        ...
                        <option>Item</option><option>Item Discount Group</option><option>Resource</option>
                      </select>
                    </td>
                    <td className="px-3 py-2">
                      <input type="text" value={l.productNo} onChange={e => updateLine(i, 'productNo', e.target.value)}
                        placeholder="ITEM-001" className="w-28 px-2 py-1 rounded bg-zinc-900 border border-zinc-700 text-xs text-zinc-100 font-mono focus:outline-none focus:border-blue-500" />
                    </td>
                    <td className="px-3 py-2">
                      <input type="text" value={l.description} onChange={e => updateLine(i, 'description', e.target.value)}
                        placeholder="Description" className="w-40 px-2 py-1 rounded bg-zinc-900 border border-zinc-700 text-xs text-zinc-100 focus:outline-none focus:border-blue-500" />
                    </td>
                    <td className="px-3 py-2">
                      <input type="text" value={l.unitOfMeasure} onChange={e => updateLine(i, 'unitOfMeasure', e.target.value)}
                        placeholder="each" className="w-20 px-2 py-1 rounded bg-zinc-900 border border-zinc-700 text-xs text-zinc-100 focus:outline-none focus:border-blue-500" />
                    </td>
                    <td className="px-3 py-2">
                      <input type="number" step="1" min="0" value={l.minQty} onChange={e => updateLine(i, 'minQty', Number(e.target.value))}
                    ...
                    <td className="px-3 py-2">
                      <input type="number" step="0.01" min="0" value={l.unitPrice} onChange={e => updateLine(i, 'unitPrice', Number(e.target.value))}
          ...
          <div className="px-5 py-2.5 border-t border-zinc-800/40 text-[11px] text-zinc-600">{lines.length} line{lines.length !== 1 ? 's' : ''}</div>
        </div>
      </div>
    </div>
  )
