...
export default function SalesReturnOrdersPage() {
  const [orders, setOrders] = useState<SalesReturnOrder[]>([])
  ...
  const load = useCallback(async (st: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/sales/return-orders?${params}`)
  ...
  const filtered = orders.filter(o => {
    ...
    return o.returnNo.toLowerCase().includes(s) || (o.sellToCustomerName ?? '').toLowerCase().includes(s)
  })
    ...
    <div className="min-h-[100dvh] bg-[#0f0f1a] text-zinc-100">
      ...
      <div className="border-b border-zinc-800 bg-[#0f0f1a] px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-zinc-400">
          ...
          <span>/</span>
          <span className="text-zinc-200">Sales Return Orders</span>
        </div>
      ...
      <div className="border-b border-zinc-800 bg-[#16213e] px-6 py-3">
        <h1 className="text-base font-semibold text-zinc-100 mb-3">Sales Return Orders</h1>
        <div className="flex items-center gap-1 flex-wrap">
          <Link href="/sales/return-orders/new"
        ...
        <div className="flex-1 p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="relative flex-1 max-w-sm">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search return orders..."
          ...
          <div className="bg-[#16213e] border border-zinc-800 rounded-lg overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800 bg-[#0f1829]">
                  <th className="w-10 px-4 py-2.5">
                    <input type="checkbox" checked={allChecked} onChange={() => setSelected(allChecked ? [] : filtered.map(o => o.id))} className="accent-blue-500" />
                  </th>
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-zinc-400 uppercase tracking-wide">Return Date</th>
              ...
              <tbody className="divide-y divide-zinc-800/60">
                ...
                {!loading && filtered.length === 0 && <tr><td colSpan={8} className="px-4 py-10 text-center text-zinc-600">No return orders found</td></tr>}
                {filtered.map(o => (
                  <tr key={o.id} className="hover:bg-zinc-800/30 transition-colors group">
                    <td className="px-4 py-3">
                      <input type="checkbox" checked={selected.includes(o.id)} onChange={() => toggleSelect(o.id)} className="accent-blue-500" />
                    </td>
                    <td className="px-4 py-3">
                      <Link href={`/sales/return-orders/${o.id}`} className="text-blue-400 hover:text-blue-300 font-mono text-xs font-medium">
                        {o.returnNo.length > 12 ? o.returnNo.slice(-12) : o.returnNo}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-zinc-200">{o.sellToCustomerName ?? <span className="text-zinc-600">—</span>}</td>
                    <td className="px-4 py-3 text-zinc-400 text-xs">{fmtDate(o.returnDate)}</td>
                    <td className="px-4 py-3 text-zinc-400 text-xs">{fmtDate(o.postingDate)}</td>
                    <td className="px-4 py-3">
                      <span className={cn('px-2 py-0.5 rounded text-xs font-medium', STATUS_COLORS[o.status] ?? 'bg-zinc-700/50 text-zinc-400')}>
                        {o.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-sm text-zinc-200">{fmtCurrency(o.totalAmount)}</td>
                    <td className="px-4 py-3 text-right">
                      <Link href={`/sales/return-orders/${o.id}`} className="text-xs text-zinc-500 hover:text-zinc-200 opacity-0 group-hover:opacity-100 transition-opacity">Edit</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
