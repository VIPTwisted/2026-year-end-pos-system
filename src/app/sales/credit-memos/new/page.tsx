...
export default function NewSalesCreditMemoPage() {
  ...
  async function save() {
    setSaving(true)
    try {
      const res = await fetch('/api/sales/credit-memos', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          taxAmount,
          totalAmount,
          creditMemoNo: `SCM-${Date.now()}`,
          lines: lines.map(({ id: _id, ...rest }) => rest),
        }),
      })
      if (!res.ok) throw new Error('Failed')
      const data = await res.json()
      router.push(`/sales/credit-memos/${data.id}`)
    } catch {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#0f0f1a] text-zinc-100">
      <div className="border-b border-zinc-800 bg-[#0f0f1a] px-6 py-3 flex items-center gap-2 text-sm text-zinc-400">
        ...
        <span>/</span>
        <Link href="/sales/credit-memos" className="hover:text-zinc-200">Sales Credit Memos</Link>
        <span>/</span>
        <span className="text-zinc-200">New</span>
      </div>

      <div className="border-b border-zinc-800 bg-[#16213e] px-6 py-3 flex items-center gap-2">
        <Link href="/sales/credit-memos" className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1e2d4a] hover:bg-[#243558] rounded text-xs font-medium text-zinc-200 transition-colors">
        ...
        <span className="px-2 py-0.5 rounded text-xs font-medium bg-blue-500/20 text-blue-400 border border-blue-500/30 ml-2">Open</span>
      </div>
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-4">
        <FastTab title="General">
          ...
          <Field label="Due Date">
            <input type="date" value={header.dueDate} onChange={e => setHeader(p => ({ ...p, dueDate: e.target.value }))} className={inputCls} />
          </Field>
          ...
          <div className="border-t border-zinc-800 px-4 py-3 flex justify-end">
            <div className="w-56 space-y-1 text-xs">
              ...
              <div className="flex justify-between text-zinc-200 font-semibold text-sm border-t border-zinc-700 pt-1">
                <span>Total</span><span className="font-mono">${totalAmount.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
