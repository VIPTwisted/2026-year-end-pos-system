...
export function BlanketSOReleaseForm({ orderId, lines }: { orderId: string; lines: Line[] }) {
  ...
  const openLines = lines.filter(l => l.quantity - l.qtyShipped > 0)
  ...
  const handleRelease = async () => {
    ...
    setError('')
    try {
      const res = await fetch(`/api/sales/blanket/${orderId}/release`, {
        method: 'POST',
  ...
  if (openLines.length === 0) {
    return (
      <Card className="bg-zinc-900 border-zinc-800">
        <CardContent className="py-6 text-center text-zinc-500 text-sm">All lines fully shipped</CardContent>
      </Card>
    )
  }
    ...
    <Card className="bg-zinc-900 border-zinc-800">
      <CardHeader>
        <CardTitle className="text-sm text-zinc-100 flex items-center gap-2">
          <Truck className="w-4 h-4" />Release Order
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
            ...
            return (
              <label key={line.id} className={`flex items-start gap-2 p-2 rounded-md cursor-pointer border transition-colors ${isSelected ? 'border-blue-600 bg-blue-900/10' : 'border-zinc-700 hover:border-zinc-600'}`}>
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-xs text-zinc-500">{formatCurrency(remaining * line.unitPrice)}</p>
                </div>
              </label>
            )
          })}
        ...
        <Button onClick={handleRelease} disabled={loading || selected.size === 0} size="sm" className="w-full">
          {loading ? 'Creating Order...' : `Release ${selected.size} line(s) → Order`}
        </Button>
      </CardContent>
    </Card>
  )
