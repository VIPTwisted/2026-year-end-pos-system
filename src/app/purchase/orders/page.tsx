...
import {
  Plus, Edit, Trash2, ChevronUp, ChevronDown, Send, PackageCheck, ChevronRight,
} from 'lucide-react'
...
export default async function PurchaseOrdersPage({
  ...
  const [orders, total, vendors] = await Promise.all([
      ...
      <main className="flex-1 overflow-auto bg-[#0f0f1a] min-h-[100dvh]">
        ...
        <div className="flex">
          ...
          <div className="w-56 shrink-0 bg-[#16213e] border-r border-zinc-800/50 min-h-[calc(100dvh-88px)] p-4 space-y-5">
            ...
            {/* Date Filters */}
            <div>
              <label className="text-[10px] uppercase tracking-widest text-zinc-500 mb-2 block">Order Date</label>
              <form method="GET" action="/purchase/orders" className="space-y-2">
            ...
            {hasFilters && (
              <Link href="/purchase/orders" className="block text-xs text-zinc-500 hover:text-zinc-300 underline mt-2">
                Clear filters
              </Link>
            )}
          </div>
          ...
          <div className="flex-1 overflow-x-auto">
            ...
            {orders.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-zinc-600">
                ...
                <Link href="/purchase/orders/new" className="text-xs text-indigo-400 hover:underline mt-2">
              ...
              <table className="w-full">
                ...
                <tbody>
                  {orders.map(order => (
                    <tr key={order.id} className="border-b border-zinc-800/30 hover:bg-[rgba(99,102,241,0.05)] transition-colors group">
                      <td className="px-3 py-2 font-mono text-[11px] text-zinc-400">
                        <Link href={`/purchase/orders/${order.id}`} className="hover:text-indigo-400 transition-colors">
                          {order.poNumber}
                        </Link>
                      </td>
                      <td className="px-3 py-2">
                        ...
                        <div className="text-[11px] text-zinc-500">{order.vendor.vendorCode}</div>
                      </td>
                      ...
                      <td className="px-3 py-2">
                        <span className={`text-[11px] px-2 py-0.5 rounded font-medium capitalize ${statusColor(order.status)}`}>
                          {order.status}
                        </span>
                      </td>
