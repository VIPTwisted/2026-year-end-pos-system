      ...
      <main className="flex-1 p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-1 bg-zinc-900/60 border border-zinc-800/50 rounded-lg p-1">
            {tabs.map(t => (
              <Link key={t.value} href={`/warehouse/picks?status=${t.value}`}>
        ...
        <div className="flex items-center gap-3 text-xs text-zinc-600">
          <span>Filter:</span>
          <span className="bg-zinc-900/40 border border-zinc-800/30 rounded px-2 py-0.5">Activity Type: Pick</span>
        </div>
        ...
        <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg overflow-hidden">
          {picks.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-zinc-500 text-[13px]">No {status.replace(/_/g, ' ')} picks</p>
            </div>
          ) : (
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-zinc-800/30 bg-zinc-900/30">
                  ...
                  <th className="text-center px-4 py-2 text-[10px] uppercase text-zinc-500 font-medium tracking-wide">Activity Type</th>
                ...
                {picks.map(p => (
                  <tr key={p.id} className="border-b border-zinc-800/30 hover:bg-zinc-800/20 transition-colors">
                    <td className="px-4 py-2">
                      <Link href={`/warehouse/picks/${p.id}`} className="font-mono text-[13px] font-semibold text-blue-400 hover:text-blue-300 hover:underline">
                        {p.pickNo}
                      </Link>
                    </td>
                    <td className="px-4 py-2 text-zinc-400">{p.assignedUserId ?? '—'}</td>
                    <td className="px-4 py-2 text-zinc-400 font-mono text-xs">{p.locationCode ?? p.store?.name ?? '—'}</td>
                    <td className="px-4 py-2 text-center">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-purple-500/10 text-purple-300 border border-purple-500/30">
                        {p.activityType}
                      </span>
                    </td>
                    <td className="px-4 py-2 text-center">
                    ...
                    <td className="px-4 py-2 text-right tabular-nums text-zinc-300">{p._count.lines}</td>
                    <td className="px-4 py-2 text-right text-zinc-600">{formatDate(p.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
