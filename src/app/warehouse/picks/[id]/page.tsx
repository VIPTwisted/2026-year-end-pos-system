...
export default async function PickDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const takeLines = pick.lines.filter(l => l.actionType === 'Take')
  const placeLines = pick.lines.filter(l => l.actionType === 'Place')

  return (
    <div className="flex flex-col min-h-[100dvh] bg-[#0f0f1a]">
      <TopBar title={`Pick ${pick.pickNo}`} />
      <main className="flex-1 p-6 space-y-6">

        {/* Header FastTab — General */}
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <h2 className="text-xl font-bold text-zinc-100 font-mono">{pick.pickNo}</h2>
                  <Badge variant={STATUS_VARIANT[pick.status] ?? 'default'} className="capitalize">
                ...
                <dl className="grid grid-cols-2 gap-x-8 gap-y-1 text-xs">
                  <div className="flex gap-2">
                    ...
                    <dd className="text-zinc-300 font-mono">{pick.locationCode ?? pick.store?.name ?? '—'}</dd>
                  </div>
                  <div className="flex gap-2">
                    <dt className="text-zinc-500 w-28">Activity Type</dt>
                    <dd className="text-zinc-300">{pick.activityType}</dd>
                  </div>
                  <div className="flex gap-2">
                    <dt className="text-zinc-500 w-28">Assigned User ID</dt>
                    <dd className="text-zinc-300">{pick.assignedUserId ?? '—'}</dd>
                  </div>
                  <div className="flex gap-2">
                    <dt className="text-zinc-500 w-28">Source</dt>
                    <dd className="text-zinc-300 capitalize">{pick.sourceType ? `${pick.sourceType} ${pick.sourceId ?? ''}`.trim() : '—'}</dd>
                  </div>
                  <div className="flex gap-2">
                    <dt className="text-zinc-500 w-28">Created</dt>
                    <dd className="text-zinc-300">{formatDate(pick.createdAt)}</dd>
                  </div>
                  {pick.completedAt && (
                    <div className="flex gap-2">
                      <dt className="text-zinc-500 w-28">Completed</dt>
                      <dd className="text-zinc-300">{formatDate(pick.completedAt)}</dd>
                    </div>
                  )}
                </dl>
              </div>
        ...
        <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg">
          <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800/40">
            <h3 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">
              Lines ({pick.lines.length})
            </h3>
          </div>
          <div className="overflow-x-auto">
            {pick.lines.length === 0 ? (
              <div className="p-8 text-center text-zinc-600 text-sm">No lines on this pick</div>
            ) : (
