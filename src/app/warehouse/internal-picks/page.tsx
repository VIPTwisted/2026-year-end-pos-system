...
function StatusChip({ status }: { status: string }) {
  const map: Record<string, string> = {
    ...
    pick_created: 'bg-amber-500/10 text-amber-400 border-amber-500/30',
...
export default async function InternalPicksPage({
  searchParams,
}: {
  ...
  const internalPicks = await prisma.warehouseInternalPick.findMany({
    where: { status },
    include: {
  ...
  const tabs = [
    ...
    { label: 'Pick Created', value: 'pick_created' },
    { label: 'Completed', value: 'completed' },
  ]

  return (
    <div className="flex flex-col min-h-[100dvh] bg-[#0f0f1a]">
      <TopBar title="Warehouse Internal Picks" />
      <main className="flex-1 p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-1 bg-zinc-900/60 border border-zinc-800/50 rounded-lg p-1">
            {tabs.map(t => (
              <Link key={t.value} href={`/warehouse/internal-picks?status=${t.value}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/warehouse/internal-picks/new">
              <button className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium bg-blue-600 hover:bg-blue-500 text-white transition-colors">
                <Plus className="w-3.5 h-3.5" /> New Internal Pick
              </button>
            </Link>
          </div>
        </div>
        ...
        <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg overflow-hidden">
          {internalPicks.length === 0 ? (
            <div className="p-12 text-center">
              <Clipboard className="w-10 h-10 text-zinc-700 mx-auto mb-3" />
              <p className="text-zinc-500 text-[13px]">No {status.replace(/_/g, ' ')} internal picks</p>
            </div>
          ) : (
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-zinc-800/30 bg-zinc-900/30">
                  ...
                  <th className="text-left px-4 py-2 text-[10px] uppercase text-zinc-500 font-medium tracking-wide">To Zone</th>
                  <th className="text-left px-4 py-2 text-[10px] uppercase text-zinc-500 font-medium tracking-wide">To Bin</th>
              ...
              <tbody>
                {internalPicks.map(ip => (
                  <tr key={ip.id} className="border-b border-zinc-800/30 hover:bg-zinc-800/20 transition-colors">
                    <td className="px-4 py-2">
                      <Link href={`/warehouse/internal-picks/${ip.id}`} className="font-mono text-[13px] font-semibold text-blue-400 hover:text-blue-300 hover:underline">
                        {ip.internalPickNo}
                      </Link>
                    </td>
                    <td className="px-4 py-2 text-zinc-400 font-mono text-xs">{ip.locationCode ?? ip.store?.name ?? '—'}</td>
                    <td className="px-4 py-2 text-zinc-400">{ip.assignedUserId ?? '—'}</td>
                    <td className="px-4 py-2 text-zinc-400 font-mono text-xs">{ip.toZoneCode ?? '—'}</td>
                    <td className="px-4 py-2 text-zinc-400 font-mono text-xs">{ip.toBinCode ?? '—'}</td>
                    <td className="px-4 py-2 text-center">
                      <StatusChip status={ip.status} />
                    </td>
                    <td className="px-4 py-2 text-right tabular-nums text-zinc-300">{ip._count.lines}</td>
                    <td className="px-4 py-2 text-right text-zinc-600">{formatDate(ip.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
