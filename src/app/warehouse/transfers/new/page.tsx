...
export default function NewTransferOrderPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (form.fromStoreId === form.toStoreId) {
      alert('Transfer-from and Transfer-to cannot be the same location')
      return
    }
    setSaving(true)
    try {
      const res = await fetch('/api/warehouse/transfers', {
        ...
        body: JSON.stringify({
          ...form,
          lines: lines.filter(l => l.productId || l.description),
        }),
      })
      if (!res.ok) throw new Error('Failed')
      const doc = await res.json()
      router.push(`/warehouse/transfers/${doc.id}`)
    } catch {
          ...
          <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg">
            ...
            <div className="p-5 grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-400">Transfer-from *</Label>
                <select
                  required
                  value={form.fromStoreId}
                  onChange={e => setField('fromStoreId', e.target.value)}
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-400">Transfer-to *</Label>
                <select
                  required
                  value={form.toStoreId}
                  onChange={e => setField('toStoreId', e.target.value)}
                  className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-blue-500"
                >
                  <option value="">Select location…</option>
                  {stores.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-400">In-Transit Code</Label>
                <Input value={form.inTransitCode} onChange={e => setField('inTransitCode', e.target.value)} placeholder="Transit location code…" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-400">Shipment Date</Label>
                <Input type="date" value={form.shipmentDate} onChange={e => setField('shipmentDate', e.target.value)} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-zinc-400">Receipt Date</Label>
              ...
              {lines.map((line, i) => (
                <div key={i} className="grid grid-cols-12 gap-3 p-3 bg-zinc-900/40 rounded-lg border border-zinc-800/40">
                  <div className="col-span-4 space-y-1">
                    ...
                    <select
                      ...
                      onChange={e => {
                        ...
                        if (prod) setLine(i, 'description', prod.name)
                      }}
                  ...
                  <div className="col-span-3 space-y-1">
                    ...
                    <Input value={line.description} onChange={e => setLine(i, 'description', e.target.value)} placeholder="Description…" className="h-8 text-xs" />
                  </div>
