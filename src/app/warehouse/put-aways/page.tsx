...
export default async function PutAwaysPage({
  searchParams,
}: {
  ...
  return (
    <div className="flex flex-col min-h-[100dvh] bg-[#0f0f1a]">
      <TopBar title="Warehouse Put-Aways" />
      <main className="flex-1 p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-1 bg-zinc-900/60 border border-zinc-800/50 rounded-lg p-1">
            {tabs.map(t => (
              <Link key={t.value} href={`/warehouse/put-aways?status=${t.value}`}>
        ...
        <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg overflow-hidden">
          {putAways.length === 0 ? (
            <div className="p-12 text-center">
              <ArrowDownToLine className="w-10 h-10 text-zinc-700 mx-auto mb-3" />
              <p className="text-zinc-500 text-[13px]">No {status.replace(/_/g, ' ')} put-aways</p>
              <p className="text-zinc-600 text-xs mt-1">Put-aways are created from Warehouse Receipts</p>
            </div>
          ) : (
                ...
                {putAways.map(p => (
                  <tr key={p.id} className="border-b border-zinc-800/30 hover:bg-zinc-800/20 transition-colors">
                    <td className="px-4 py-2">
                      <Link href={`/warehouse/put-aways/${p.id}`} className="font-mono text-[13px] font-semibold text-blue-400 hover:text-blue-300 hover:underline">
                        {p.putAwayNo}
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-2 text-zinc-400 capitalize text-xs">{p.sourceType ?? '—'}</td>
