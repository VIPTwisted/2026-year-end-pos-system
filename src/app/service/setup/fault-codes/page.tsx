...
export default async function FaultCodesPage() {
  ...
  return (
    <>
      <TopBar title="Fault Codes" />
      <main className="flex-1 p-6 overflow-auto space-y-5 max-w-3xl">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-base font-semibold text-zinc-100">Fault Codes</h1>
        ...
        <Card>
          <CardContent className="pt-5 pb-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-zinc-400 mb-3">Add New Fault Code</p>
            <NewCodeForm endpoint="/api/service/setup/fault-codes" />
          </CardContent>
        </Card>
        ...
        {codes.length === 0 ? (
          <div className="flex items-center justify-center py-16 text-zinc-600">
            <p className="text-sm">No fault codes defined yet.</p>
          </div>
        ) : (
