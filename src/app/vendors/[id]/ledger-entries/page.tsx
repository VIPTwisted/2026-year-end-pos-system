...
export default async function VendorLedgerEntriesPage({
  params,
  searchParams,
}: {
  ...
  function buildHref(overrides: Record<string, string>) {
    ...
    return `/vendors/${id}/ledger-entries?${new URLSearchParams(merged).toString()}`
  }
    ...
    <>
      ...
      <main className="flex-1 overflow-auto bg-[#0f0f1a] min-h-[100dvh]">
        ...
        <div className="bg-[#16213e] border-b border-zinc-800/50 px-6 py-2 text-xs text-zinc-500 flex items-center gap-1.5">
          ...
          <span>/</span>
          <Link href={`/vendors/${id}`} className="hover:text-zinc-300 transition-colors">
        ...
        <div className="bg-[#16213e] border-b border-zinc-800/50 px-6 py-3 flex items-center gap-3">
          <Link href={`/vendors/${id}`} className="text-zinc-500 hover:text-zinc-300 transition-colors">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <BookOpen className="w-5 h-5 text-zinc-400" />
          <h1 className="text-lg font-bold text-zinc-100">
            Vendor Ledger Entries — {vendor.vendorCode} {vendor.name}
          </h1>
        </div>
