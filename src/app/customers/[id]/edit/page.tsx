...
export default function EditCustomerPage() {
    ...
    fetch(`/api/customers/${id}`)
      ...
      .then((data: CustomerData | null) => {
        ...
        setForm(prev => ({
          ...
          blocked:          !data.isActive ? 'All' : data.creditStatus === 'hold' ? 'Invoice' : 'None',
        }))
      })
  ...
  if (notFound) {
    return (
      <>
        ...
        <main className="flex-1 bg-[#0f0f1a] min-h-[100dvh] flex flex-col items-center justify-center gap-3">
          <p className="text-zinc-400">Customer not found.</p>
          <Link href="/customers" className="text-xs text-blue-400 hover:underline">Back to Customers</Link>
        </main>
      </>
    )
  }
  ...
  return (
    <>
      <TopBar title={`${form.firstName} ${form.lastName} — Edit`} />
      <main className="flex-1 overflow-auto bg-[#0f0f1a] min-h-[100dvh]">
        ...
        <form id="edit-customer-form" onSubmit={handleSubmit} className="px-6 py-4 flex gap-4">
          ...
          <div className="w-64 shrink-0">
            <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg p-4">
              ...
              <p className="text-xs text-zinc-600">
                Edit customer details and click <strong className="text-zinc-400">Save</strong> to apply.
                Click <strong className="text-zinc-400">Discard</strong> to cancel without saving.
              </p>
            </div>
          </div>
        </form>
