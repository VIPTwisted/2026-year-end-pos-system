...
export default async function CustomerApplyEntriesPage({
  searchParams,
}: {
  ...
  let invoices: Awaited<ReturnType<typeof prisma.customerInvoice.findMany>> = []
  let customer: (typeof customers)[0] | null = null

  if (customerId) {
    customer = customers.find(c => c.id === customerId) ?? null
    invoices = await prisma.customerInvoice.findMany({
  ...
  return (
    <>
      <TopBar title="Apply Customer Entries" />
      <main className="flex-1 overflow-auto bg-[#0f0f1a] min-h-[100dvh]">
        ...
        <div className="bg-[#16213e] border-b border-zinc-800/50 px-6 py-3 flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-zinc-100">Apply Customer Entries</h1>
        </div>
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg p-4">
            <label className="text-[10px] uppercase tracking-widest text-zinc-500 mb-2 block">Customer No.</label>
            ...
            <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg overflow-hidden">
              <div className="px-4 py-3 bg-zinc-900/40 border-b border-zinc-800/50 flex items-center justify-between">
                <span className="text-sm font-semibold text-zinc-200">
                  Open Entries — {customer.firstName} {customer.lastName}
                </span>
              ...
              {invoices.length === 0 ? (
                <div className="flex items-center justify-center py-12 text-zinc-600">
                  <p className="text-sm">No open entries for this customer.</p>
                </div>
              ) : (
                <>
