...
type LineRow = { no: string; description: string; qty: number; unitCost: number; discountPct: number; amount: number }
...
export default function NewPurchaseQuotePage() {
  ...
  const [lines, setLines] = useState<LineRow[]>([
    { no: '', description: '', qty: 1, unitCost: 0, discountPct: 0, amount: 0 },
  ])
  ...
  function updateLine(idx: number, field: keyof LineRow, value: string | number) {
    setLines(prev => {
      ...
      row.amount = Number(row.qty) * Number(row.unitCost) * (1 - Number(row.discountPct) / 100)
      next[idx] = row
      return next
    })
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/purchasing/orders', {
        ...
        body: JSON.stringify({
          vendorId, orderDate: quoteDate, expectedDate: validUntil, purchaser, notes,
          isQuote: true,
          lines: lines.filter(l => l.description).map(l => ({
            productName: l.description, sku: l.no,
            qtyOrdered: l.qty, unitCost: l.unitCost, lineTotal: l.amount,
          })),
        }),
      })
      if (!res.ok) { const d = await res.json(); throw new Error(d.error ?? 'Failed') }
      const q = await res.json()
      router.push(`/purchasing/quotes/${q.id}`)
    } catch (err) {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#0f0f1a] text-zinc-100">
      ...
      <div className="border-b border-zinc-800 bg-[#16213e] px-4 py-2 flex items-center gap-2">
        ...
        <Link href="/purchasing/quotes"><button className="px-3 py-1.5 text-xs bg-zinc-700 hover:bg-zinc-600 text-zinc-200 rounded font-medium">Discard</button></Link>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4 max-w-5xl">
        <details open className="bg-[#16213e] border border-zinc-800/50 rounded-lg">
          ...
          <div className="px-4 pb-4 grid grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Quote Date</label>
              <input type="date" value={quoteDate} onChange={e => setQuoteDate(e.target.value)} className="w-full bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm text-zinc-200" />
            </div>
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Valid Until</label>
              <input type="date" value={validUntil} onChange={e => setValidUntil(e.target.value)} className="w-full bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm text-zinc-200" />
            </div>
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Purchaser</label>
              <input type="text" value={purchaser} onChange={e => setPurchaser(e.target.value)} className="w-full bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm text-zinc-200" />
            </div>
        ...
        <details open className="bg-[#16213e] border border-zinc-800/50 rounded-lg">
          ...
          <div className="px-4 pb-4">
            <table className="w-full text-sm mb-3">
              <thead>
                <tr className="border-b border-zinc-700 text-zinc-400 text-xs uppercase tracking-wide">
                  ...
                  <th className="text-right pb-2 pr-2 font-medium w-28">Unit Cost</th>
              ...
              <tbody className="divide-y divide-zinc-800">
                {lines.map((line, idx) => (
                  <tr key={idx}>
                    <td className="py-1.5 pr-2"><input type="text" value={line.no} onChange={e => updateLine(idx, 'no', e.target.value)} placeholder="Item No." className="w-full bg-zinc-800 border border-zinc-700 rounded px-1.5 py-1 text-xs text-zinc-200" /></td>
                    ...
                    <td className="py-1.5 pr-2"><input type="number" min="0" step="0.01" value={line.unitCost} onChange={e => updateLine(idx, 'unitCost', Number(e.target.value))} className="w-full bg-zinc-800 border border-zinc-700 rounded px-1.5 py-1 text-xs text-zinc-200 text-right" /></td>
                    <td className="py-1.5 pr-2"><input type="number" min="0" max="100" step="0.1" value={line.discountPct} onChange={e => updateLine(idx, 'discountPct', Number(e.target.value))} className="w-full bg-zinc-800 border border-zinc-700 rounded px-1.5 py-1 text-xs text-zinc-200 text-right" /></td>
                    <td className="py-1.5 pr-2 text-right text-emerald-400 font-semibold tabular-nums text-xs">{line.amount.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</td>
            ...
            <button type="button" onClick={() => setLines(prev => [...prev, { no: '', description: '', qty: 1, unitCost: 0, discountPct: 0, amount: 0 }])} className="flex items-center gap-1.5 text-xs text-blue-400 hover:text-blue-300">
