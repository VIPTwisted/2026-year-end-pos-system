import { NextResponse } from 'next/server'
import { prisma } from '@/lib/prisma'

type Classification = 'active' | 'slow_moving' | 'aging' | 'dead_stock'

function classify(daysSince: number): Classification {
  if (daysSince <= 30) return 'active'
  if (daysSince <= 90) return 'slow_moving'
  if (daysSince <= 180) return 'aging'
  return 'dead_stock'
}

export async function GET() {
  try {
    const now = new Date()

    // All products that have stock > 0 across any inventory record
    const products = await prisma.product.findMany({
      where: {
        inventory: { some: { quantity: { gt: 0 } } },
      },
      include: {
        category: true,
        inventory: true,
        orderItems: {
          include: {
            order: {
              select: { status: true, createdAt: true },
            },
          },
          orderBy: { order: { createdAt: 'desc' } },
        },
      },
    })

    // 30-day and 90-day cutoff timestamps
    const cutoff30 = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000)
    const cutoff90 = new Date(now.getTime() - 90 * 24 * 60 * 60 * 1000)

    const items = products.map((product) => {
      // Aggregate stock across all inventory records
      const currentQty = product.inventory.reduce((sum, inv) => sum + inv.quantity, 0)

      // Filter to completed / paid orders only
      const completedItems = product.orderItems.filter(
        (oi) => oi.order.status === 'completed' || oi.order.status === 'paid',
      )

      // Last sale date: most recent completed order containing this product
      let lastSoldDate: Date | null = null
      if (completedItems.length > 0) {
        // orderItems already ordered desc by createdAt on the order
        lastSoldDate = completedItems[0].order.createdAt
      }

      // Reference date for daysSinceLastSold
      const refDate = lastSoldDate ?? product.createdAt
      const daysSinceLastSold = Math.floor(
        (now.getTime() - refDate.getTime()) / (1000 * 60 * 60 * 24),
      )

      // Velocity: units sold in 30d and 90d windows
      const velocity30d = completedItems
        .filter((oi) => oi.order.createdAt >= cutoff30)
        .reduce((sum, oi) => sum + oi.quantity, 0)

      const velocity90d = completedItems
        .filter((oi) => oi.order.createdAt >= cutoff90)
        .reduce((sum, oi) => sum + oi.quantity, 0)

      const unitCost = product.costPrice ?? 0
      const stockValue = currentQty * unitCost
      const classification = classify(daysSinceLastSold)

      return {
        productId: product.id,
        name: product.name,
        sku: product.sku,
        category: product.category?.name ?? null,
        currentQty,
        unitCost,
        stockValue,
        lastSoldDate: lastSoldDate ? lastSoldDate.toISOString() : null,
        daysSinceLastSold,
        classification,
        velocity30d,
        velocity90d,
      }
    })

    // Summary
    const totalStockValue = items.reduce((s, i) => s + i.stockValue, 0)
    const activeValue = items
      .filter((i) => i.classification === 'active')
      .reduce((s, i) => s + i.stockValue, 0)
    const slowMovingValue = items
      .filter((i) => i.classification === 'slow_moving')
      .reduce((s, i) => s + i.stockValue, 0)
    const agingValue = items
      .filter((i) => i.classification === 'aging')
      .reduce((s, i) => s + i.stockValue, 0)
    const deadStockValue = items
      .filter((i) => i.classification === 'dead_stock')
      .reduce((s, i) => s + i.stockValue, 0)
    const deadStockCount = items.filter((i) => i.classification === 'dead_stock').length

    return NextResponse.json({
      items,
      summary: {
        totalStockValue,
        activeValue,
        slowMovingValue,
        agingValue,
        deadStockValue,
        deadStockCount,
      },
    })
  } catch (e) {
    console.error(e)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
