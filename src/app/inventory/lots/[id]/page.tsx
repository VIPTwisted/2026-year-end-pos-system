...
export default function LotDetailPage() {
  ...
  const loadLot = useCallback(() => {
    setLoading(true)
    fetch(`/api/inventory/lots/${id}`)
      .then(r => {
        if (!r.ok) throw new Error('Not found')
        return r.json() as Promise<LotDetail>
      })
      .then(setLot)
  ...
  async function handleAddMovement(e: React.FormEvent) {
    ...
    if (isNaN(qty) || qty === 0) {
      notify('Enter a valid non-zero quantity', 'err')
      return
    }

    setSaving(true)
    try {
      const res = await fetch(`/api/inventory/lots/${id}/movements`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          type: movForm.type,
          quantity: qty,
          reference: movForm.reference.trim() || undefined,
      ...
      if (!res.ok) {
        ...
        notify(data.error ?? 'Failed to add movement', 'err')
        return
      }

      notify('Movement recorded', 'ok')
  ...
  async function toggleBlock() {
    ...
    try {
      ...
      notify(lot.isBlocked ? 'Lot unblocked' : 'Lot blocked', 'ok')
      loadLot()
    } catch {
      notify('Failed to update lot', 'err')
    } finally {
      setBlocking(false)
    }
  }
  ...
  if (loading) {
    return (
      <>
        <TopBar title="Lot Detail" breadcrumb={[{ label: 'Inventory', href: '/inventory' }, { label: 'Lots', href: '/inventory/lots' }]} />
  ...
  if (!lot) {
    return (
      <>
        <TopBar title="Lot Not Found" breadcrumb={[{ label: 'Inventory', href: '/inventory' }, { label: 'Lots', href: '/inventory/lots' }]} />
        <main className="flex-1 flex items-center justify-center bg-[#0f0f1a] min-h-[100dvh]">
          <div className="text-center">
            <p className="text-zinc-400 mb-3">Lot not found</p>
            <Link href="/inventory/lots" className="text-blue-400 hover:text-blue-300 text-[13px]">Back to Lots</Link>
          </div>
        </main>
      </>
    )
      ...
      <main className="flex-1 overflow-auto bg-[#0f0f1a] min-h-[100dvh]">
        <div className="max-w-4xl mx-auto p-6 space-y-6">
          ...
          <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg p-5">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[12px] text-zinc-500 mt-0.5">
                    <Link href={`/products/${lot.product.id}`} className="text-blue-400 hover:text-blue-300">
                      {lot.product.name}
                    </Link>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={toggleBlock}
                  disabled={blocking || lot.isExpired}
                  className="inline-flex items-center gap-1.5 h-7 px-3 rounded text-[12px] font-medium border border-zinc-700 text-zinc-400 hover:text-zinc-200 hover:border-zinc-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <InfoCard label="Supplier" value={lot.supplier?.name ?? '—'} />
            <InfoCard label="Qty Received" value={lot.quantity.toLocaleString()} />
            <InfoCard label="Qty On Hand" value={lot.quantityOnHand.toLocaleString()} highlight={lot.quantityOnHand === 0 ? 'dim' : 'normal'} />
            <InfoCard
              label="Qty Consumed"
              value={(lot.quantity - lot.quantityOnHand).toLocaleString()}
            />
            <InfoCard
              label="Manufactured"
            ...
            <InfoCard
              ...
              highlight={status === 'expired' ? 'danger' : status === 'expiring' ? 'warning' : 'normal'}
            />
            <InfoCard
              label="Received"
              value={new Date(lot.receivedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            />
            <InfoCard
              label="Created"
              value={new Date(lot.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            />
          </div>
          ...
          <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg overflow-hidden">
            <div className="flex items-center gap-2.5 px-4 py-3 border-b border-zinc-800/40">
              ...
              <span className="text-[13px] font-medium text-zinc-300">Movement History</span>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-zinc-800/40">
                      ...
                      <th className="text-right text-[10px] font-semibold uppercase tracking-widest text-zinc-500 py-2.5 px-4">Quantity</th>
                    ...
                    {lot.movements.map(m => (
                      <tr key={m.id} className="border-b border-zinc-800/40 last:border-0 hover:bg-zinc-800/20 transition-colors">
                        ...
                        <td className="py-2.5 px-4 text-right">
                          <span className={`font-semibold tabular-nums text-[13px] ${m.type === 'sale' || m.type === 'transfer' ? 'text-red-400' : m.type === 'receipt' || m.type === 'return' ? 'text-emerald-400' : 'text-zinc-300'}`}>
                            {m.type === 'sale' || m.type === 'transfer' ? '-' : '+'}{Math.abs(m.quantity)}
                          </span>
                        </td>
                        <td className="py-2.5 px-4 text-[13px] font-mono text-zinc-500">
                          {m.reference ?? <span className="text-zinc-700">—</span>}
                        </td>
                        ...
                        <td className="py-2.5 px-4 text-[13px] text-zinc-400">
                          ...
                          <span className="text-zinc-600 ml-1.5 text-[11px]">
...
function InfoCard({
  ...
  label: string
  value: string
  highlight?: 'normal' | 'dim' | 'danger' | 'warning'
}) {
  const valueClass =
    highlight === 'danger'
      ? 'text-red-400'
