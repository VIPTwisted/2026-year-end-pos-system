...
export default function NewTaxCodePage() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error ?? 'Failed to create tax code.')
        setSaving(false)
        return
      }

      router.push('/finance/tax')
    } catch {
  ...
  return (
    <>
      <TopBar title="New Tax Code" />
      <main className="flex-1 p-6 overflow-auto space-y-6 max-w-2xl">
        <div>
          <h2 className="text-lg font-semibold text-zinc-100">New Tax Code</h2>
          <p className="text-sm text-zinc-500">Define a tax rate to apply to transactions</p>
        </div>

        <Card>
          <div className="px-5 py-3 border-b border-zinc-800 bg-zinc-900/60 rounded-t-lg">
            <h3 className="text-sm font-semibold text-zinc-200">Tax Code Details</h3>
          </div>
          <CardContent className="pt-5 pb-6 space-y-4">
            ...
            <div>
              <label className="block text-xs text-zinc-500 uppercase tracking-wide mb-1.5">Tax Type *</label>
              <select
                value={form.taxType}
            ...
            <div>
              ...
              <Input
                ...
                placeholder="Standard state sales tax applied to retail transactions"
                className="bg-zinc-900 border-zinc-700 text-zinc-100"
              />
            </div>
            ...
            <div className="flex items-center gap-3 pt-1">
              ...
              <label htmlFor="isActive" className="text-sm text-zinc-300 cursor-pointer">
                Active (can be applied to transactions)
              </label>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex items-center gap-3">
          <Button onClick={handleSave} disabled={saving}>
            {saving ? (
              ...
              <><Save className="w-4 h-4 mr-1" />Create Tax Code</>
            )}
          </Button>
          <Button
            variant="outline"
