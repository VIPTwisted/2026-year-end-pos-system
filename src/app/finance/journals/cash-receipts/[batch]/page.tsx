...
import {
  CheckCircle, Plus, Trash2, RefreshCw, LinkIcon, ChevronDown,
  FileCheck, Download,
} from 'lucide-react'
...
const CR_BATCHES = ['CASHRECPT', 'CASHREC-01', 'DEPOSITS', 'BANK-RECV']
const DOC_TYPES = ['Payment', 'Refund', '']
const ACCOUNT_TYPES = ['Customer', 'G/L Account', 'Vendor', 'Bank Account']
...
function EditCell({
  ...
  const cls = `h-7 w-full bg-transparent text-xs text-zinc-100 px-2 focus:outline-none focus:bg-emerald-900/20 border-none ${right ? 'text-right' : ''}`
...
export default function CashReceiptJournalEditorPage() {
  ...
  const [selectedBatch, setSelectedBatch] = useState('CASHRECPT')
  ...
  const load = useCallback(() => {
    setLoading(true)
    fetch(`/api/finance/journals/general?batchId=${batchId}`)
      .then(r => r.json())
      .then(d => {
        if (d.batch) {
          setBatch(d.batch)
          setSelectedBatch(d.batch.batchName)
          const existing: EditRow[] = d.batch.lines.map((l: CRLine) => ({
            id: l.id,
            postingDate: l.postingDate?.slice(0, 10) ?? new Date().toISOString().slice(0, 10),
            documentType: l.documentType ?? 'Payment',
            documentNo: '',
            accountType: l.accountType ?? 'Customer',
            accountNo: l.accountNo ?? '',
            customerName: l.accountName ?? '',
  ...
  const totalReceipts = rows.reduce((s, r) => s + (parseFloat(String(r.amount)) || 0), 0)
        ...
        dirty.map(r =>
          fetch('/api/finance/journals/general', {
            ...
            body: JSON.stringify({
              ...
              debitAmount: 0,
              creditAmount: parseFloat(String(r.amount)) || 0,
  ...
  const postJournal = async () => {
    ...
    setPosting(true)
    try {
      const res = await fetch(`/api/finance/journals/general?batchId=${batchId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'post' }),
      })
      if (!res.ok) throw new Error()
      notify('Cash receipt journal posted')
      load()
    } catch {
    ...
    <div className="flex flex-col min-h-[100dvh] bg-[#0f0f1a]">
      <TopBar
        ...
        breadcrumb={[
          ...
          { label: 'Cash Receipt Journals', href: '/finance/journals/cash-receipts' },
        ]}
        actions={
          !isPosted ? (
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              <button className="flex items-center gap-1 px-2.5 py-1.5 rounded text-[11px] font-medium bg-violet-700/40 text-violet-300 border border-violet-700/40 hover:bg-violet-700/60 transition-colors">
                <LinkIcon className="w-3 h-3" /> Apply Entries
              </button>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-4 px-5 py-3 bg-[#0f0f1a] border-b border-zinc-800/60">
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <select value={selectedBatch} onChange={e => setSelectedBatch(e.target.value)} disabled={isPosted}
                ...
                {CR_BATCHES.map(b => <option key={b}>{b}</option>)}
              </select>
          ...
          {!isPosted && (
            <button onClick={() => setRows(r => [...r, emptyRow()])}
              className="ml-auto flex items-center gap-1 px-2.5 py-1 rounded text-[11px] font-medium bg-emerald-600/20 text-emerald-400 border border-emerald-600/30 hover:bg-emerald-600/30 transition-colors">
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
            ...
            <table ref={tableRef} className="w-full text-xs border-collapse" style={{ minWidth: 1300 }}>
              <thead className="sticky top-0 z-10">
                <tr className="bg-[#0d0e24] border-b border-zinc-700">
                  <th className="w-8 px-2 py-2.5 border-r border-zinc-800/60" />
                  {['Posting Date', 'Doc Type', 'Doc No.', 'Account Type', 'Account No.', 'Customer Name', 'Description', 'Amount', 'Applies-to Type', 'Applies-to No.', 'Bal. Acct Type', 'Currency'].map(h => (
                ...
                {rows.map((row, ri) => {
                  ...
                  return (
                    <tr key={ri} data-row={ri}
                      className={`border-b border-zinc-800/40 transition-colors ${isSelected ? 'bg-emerald-900/10' : isLast && !isPosted ? 'bg-zinc-900/20' : 'hover:bg-zinc-800/10'}`}>
                      ...
                      {isPosted ? (
                        <>
                          ...
                          <td className="border-r border-zinc-800/60 px-2 py-1.5 font-mono text-zinc-400">{row.documentNo || '—'}</td>
                          ...
                          <td className="border-r border-zinc-800/60 px-2 py-1.5 text-zinc-400">{row.customerName || '—'}</td>
                          <td className="border-r border-zinc-800/60 px-2 py-1.5 text-zinc-400 max-w-[160px] truncate">{row.description || '—'}</td>
                          <td className="border-r border-zinc-800/60 px-2 py-1.5 font-mono text-right text-emerald-300">{fmtAmt(row.amount)}</td>
                        ...
                        <>
                          ...
                          <EditCell value={row.documentNo} onChange={v => updateRow(ri, 'documentNo', v)} onTab={handleTab(ri, 2)} placeholder="DOC-001" width="90px" />
                          <EditCell type="select" options={ACCOUNT_TYPES} value={row.accountType} onChange={v => updateRow(ri, 'accountType', v)} onTab={handleTab(ri, 3)} width="110px" />
                          <EditCell value={row.accountNo} onChange={v => updateRow(ri, 'accountNo', v)} onTab={handleTab(ri, 4)} placeholder="C-001" width="90px" />
                          <EditCell value={row.customerName} onChange={v => updateRow(ri, 'customerName', v)} onTab={handleTab(ri, 5)} placeholder="Customer name" width="140px" />
                          ...
                          <EditCell type="select" options={APPLIES_TYPES} value={row.appliesToDocType} onChange={v => updateRow(ri, 'appliesToDocType', v)} onTab={handleTab(ri, 8)} width="110px" />
                          <EditCell value={row.appliesToDocNo} onChange={v => updateRow(ri, 'appliesToDocNo', v)} onTab={handleTab(ri, 9)} placeholder="INV-001" width="90px" />
                          <EditCell type="select" options={BAL_TYPES} value={row.balAccountType} onChange={v => updateRow(ri, 'balAccountType', v)} onTab={handleTab(ri, 10)} width="120px" />
                          <EditCell type="select" options={CURRENCIES} value={row.currency} onChange={v => updateRow(ri, 'currency', v)} onTab={handleTab(ri, 11)} width="70px" />
                        </>
                      )}
                    </tr>
                  )
        ...
        <div className="border-t border-zinc-800/60 bg-[#0d0e24] px-5 py-3 flex items-center gap-8">
          <div className="flex items-center gap-2 text-xs">
            <span className="text-zinc-500 uppercase tracking-wider text-[10px] font-semibold">Total Receipts</span>
            <span className="font-mono font-bold text-emerald-300 tabular-nums">{fmtCurr(totalReceipts)}</span>
          </div>
          <div className="flex items-center gap-2 text-xs">
