...
export default async function ICOutboxPage() {
        ...
        <div className="max-w-7xl mx-auto p-6 space-y-6">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <h2 className="text-xl font-bold text-zinc-100">IC Outbox</h2>
              <p className="text-xs text-zinc-500 mt-0.5">Outgoing intercompany transactions sent to partner companies</p>
            </div>
          ...
          {transactions.length === 0 ? (
            <div className="bg-[#16213e] border border-zinc-800/50 rounded-lg flex flex-col items-center justify-center py-14 text-zinc-600">
              ...
              <p className="text-[13px]">No outgoing transactions yet.</p>
            </div>
          ) : (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="border-b border-zinc-800/60">
                    <tr>
                      {['Transaction No.', 'IC Partner', 'Document Type', 'Document No.', 'Amount', 'Currency', 'Posting Date', 'Status'].map(h => (
                        <th key={h} className={`px-4 py-3 text-[10px] font-semibold uppercase tracking-widest text-zinc-500 whitespace-nowrap ${h === 'Amount' ? 'text-right' : 'text-left'}`}>{h}</th>
                      ))}
                    </tr>
                  </thead>
