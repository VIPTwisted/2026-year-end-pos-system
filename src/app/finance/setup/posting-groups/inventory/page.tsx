...
export default async function InventoryPostingGroupsPage() {
  const groups = await prisma.inventoryPostingGroup.findMany({ orderBy: { code: 'asc' } })

  return (
    <div className="min-h-[100dvh] bg-[#0f0f1a]">
      <TopBar title="Inventory Posting Groups" />
      <main className="p-6 space-y-4">
        <div className="flex items-center gap-2">
          <Link
            href="/finance/setup/posting-groups/inventory/new"
        ...
        <div className="bg-[#16213e] border border-zinc-700 rounded-lg p-4">
          <p className="text-xs text-zinc-400">{groups.length} Inventory Posting Group{groups.length !== 1 ? 's' : ''} — linked to Inventory Posting Setup</p>
        </div>
        ...
        <div className="bg-[#16213e] border border-zinc-700 rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {groups.length === 0 && (
                <tr><td colSpan={2} className="px-4 py-8 text-center text-zinc-500 text-sm">No inventory posting groups found.</td></tr>
              )}
              {groups.map((g, i) => (
                <tr key={g.id} className={`border-b border-zinc-800 hover:bg-zinc-800/40 transition-colors ${i % 2 === 0 ? '' : 'bg-zinc-900/20'}`}>
                  <td className="px-4 py-2.5">
                    <Link href={`/finance/setup/posting-groups/inventory/new?id=${g.id}`} className="text-blue-400 hover:underline font-mono text-xs">{g.code}</Link>
                  </td>
